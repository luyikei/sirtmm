...
std::pair<arma::vec, arma::mat> item_deriv(int y, double theta, double b, double a, double c, const arma::rowvec &g, int K, bool extended)
{
  ...
  const int size = extended ? g.n_elem + 2 : g.n_elem + 1;
  const unsigned int C_IDX = 1 + g.n_elem;
  zd1.zeros(size);
  H.zeros(size, size);
...
double item_single(unsigned int y, double theta, double b, double a, double c, const arma::rowvec &g, int K)
{
  ...
  for (unsigned int k = 1; k <= y; ++k)
  {
    ...
    den = den * ((K - k) * (1 - c) / (K - 1.0) + c + exp(Z(k, theta, b, a, g)));
  }
  return num / den;
}
                   ...
                   double lambda, double lambda2, int maxIter = 50)
{
  ...
  arma::mat V;
  auto gMat = Rcpp::as<arma::mat>(gs);
  const unsigned int size = extended ? gMat.n_cols + 2 : gMat.n_cols + 1;

  const bool nog = gMat.n_cols == 1;
  const unsigned int ng = gMat.n_cols - 1;
  const unsigned int C_IDX = 1 + gMat.n_cols;
  ...
  for (int j = 0; j < M; ++j)
  {
    ...
    for (int i = 0; i < maxIter; ++i)
    {
      ...
      for (int f = 0; f < PN; ++f)
      {
        for (int k = 0; k < K; ++k)
        {
          auto deriv = item_deriv(k + 1, points[f], v(0), v(1), extended ? v(C_IDX) : 1.0 / K, gRow, K, extended);
          auto d = deriv.first;
          auto H = deriv.second;
          t += rhat[k + f * K + j * K * PN] * d;
          V += -nhat[f] * item_single(k + 1, points[f], v(0), v(1), extended ? v(C_IDX) : 1.0 / K, gRow, K) * H;
        }
      }
             ...
             double lambda, double lambda2, int ngs, int maxEMIter = 10, int maxNRIter = 50, bool verbose = false, bool exntended = true)
{
  ...
  for (int iter = 0; iter < maxEMIter; ++iter)
  {
    ...
    auto ret = fisherScoring(bs, as, cs, gs, K, rhat, nhat, points, exntended, lambda, lambda2, maxNRIter);
    bs = ret["b"];
    as = ret["a"];
    if (exntended)
      cs = ret["c"];
        ...
        const NumericMatrix &gs, int K, NumericVector points, const NumericVector &weights, bool extended)
{
  ...
  const int size = extended ? 2 + gMat.n_cols : 1 + gMat.n_cols;
